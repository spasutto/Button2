...
void Button2::setLongPressedHandler(CallbackFunction f, unsigned short delay) {
  longpress_delay = delay;
  longpressed_cb = f;
}
...
void Button2::loop() {
  ...
  if (prev_state != pressed && state == pressed) {
    down_ms = millis();
    longpressed_triggered = pressed_triggered = false;
    click_ms = down_ms;
  ...
  // is the button pressed and the time has passed for long press?
  } else if (state == pressed && !longpressed_triggered && longpressed_cb != NULL && millis() - down_ms >= longpress_delay) {
    longpressed_triggered = true;
    longpressed_cb(*this);
